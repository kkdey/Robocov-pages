...
for(nsim in 1:NUM_SIM){
  ...
  df5 = c(length(which(robocov_box_cor[which(corSigma==0)] > 0.05))/length(robocov_box_cor[which(corSigma==0)]),
          length(which(robocov_local_cor[which(corSigma==0)] > 0.05))/length(robocov_local_cor[which(corSigma==0)]),
          length(which(robocov_box_slack_cor[which(corSigma==0)] > 0.05))/length(robocov_box_slack_cor[which(corSigma==0)]),
          length(which(corshrink_cor[which(corSigma==0)] > 0.05))/length(corshrink_cor[which(corSigma==0)]),
          length(which(standard_cor[which(corSigma==0)] > 0.05))/length(standard_cor[which(corSigma==0)]),
          length(which(strimmer_cor[which(corSigma==0)] > 0.05))/length(standard_cor[which(corSigma==0)]),
          length(which(pdsoft_cor[which(corSigma==0)] > 0.05))/length(standard_cor[which(corSigma==0)])
  )
...
save(df, file = paste0("/n/groups/price/kushal/Robocov/output_sparse/robocov_sim_toeplitz_n_",
