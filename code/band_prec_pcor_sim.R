...
for(nsim in 1:NUM_SIM){
  ...
  robocov_precision = Robocov_box(data_with_missing = data_missing)

  impute_method <- "svd"
  matc=biScale(data_missing,col.scale=TRUE,row.scale=FALSE,trace=TRUE)
  fits3=softImpute(matc,rank.max=50,lambda=1,type=impute_method)
  fitted_mat <- complete(data_missing,fits3,unscale=T)

  gg = glasso::glasso(cor(data_missing, use = "pairwise.complete.obs"), rho=1e-04)
  pglasso = -cov2cor(gg$wi)
  diag(pglasso) = 1
  image(pglasso)

  df1 = c(norm(robocov_precision - pcorSigma, type = "2"),
          norm(pglasso - corSigma, type = "2"))
