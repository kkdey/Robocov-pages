...
block = chunk(1:dim(dat)[[3]], 50)
ids = block[[group]]

robomat = array(0, c(dim(dat)[2], dim(dat)[2], length(ids)))
for(n in 1:length(ids)){
  out = suppressMessages(suppressWarnings(Robocov_precision(data_with_missing = dat[,,ids[n]],
                                                            alpha = 0.1,
                                                            lambda = 1)))
  robomat[,,n] = out
...
save(robomat, file = paste0("/n/groups/price/kushal/Robocov/data/", "Robocov_prec_", group, ".rda"))
...
tot_cov = c()
for(mm in 1:50){
  tmp = get(load(paste0("/n/groups/price/kushal/Robocov/data/", "Robocov_prec_", mm, ".rda")))
...
save(tot_cov, file = "/n/groups/price/kushal/Robocov/data/Robocov_Precision_all_genes.rda")
