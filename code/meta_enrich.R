library(data.table)
library(rmeta)
annot_cell = "/n/groups/price/kushal/Mouse_Humans/data/ANNOTATIONS/GENE_SCORES/Aggregate_Aug8"
results_cell = "/n/groups/price/kushal/Mouse_Humans/data/LDSC_RESULTS/GENE_SCORES/Aggregate_Aug8/baselineLD_v2.1_post_Steven3"
base_path = "/n/groups/price/kushal/Mouse_Humans/data/ANNOTATIONS/Baselines/baselineLD_v2.1_post_Steven3"
annot_names = "FS1"
...
run_enrichment_analysis = function(annot_cell,
                                   results_cell,
                                   base_path,
                                   annotation,
                                   traits,
...
out1 = run_enrichment_analysis(annot_cell, results_cell, annotation = annot_names,
                               traits = all_traits)
out2 = run_enrichment_analysis(annot_cell, results_cell, annotation = annot_names,
                               traits = brain_traits, base_index = 90)
out3 = run_enrichment_analysis(annot_cell, results_cell, base_path,
                               annotation = annot_names,
                               traits = c(blood_traits, autoimmune_traits), base_index = NULL)
out4 = run_enrichment_analysis(annot_cell, results_cell, annotation = annot_names,
...
save(ll, file = paste0("/n/groups/price/kushal/LDSC-Average/output/", annot_names, "_enrich.rda"))
...
library(rmeta)
cell = "Chrom_Avg"
annot_cell = paste0("/n/groups/price/kushal/LDSC-Average/data/ANNOTATIONS/FULL_ANNOT_PACKET/", cell)
results_cell = paste0("/n/groups/price/kushal/LDSC-Average/data/LDSC_RESULTS/FULL_ANNOT_PACKET/", cell)
base_path = "/n/groups/price/kushal/LDSC-Average/data/ANNOTATIONS/Baselines/baselineLD_v2.1"
annot_names = list.files(results_cell)

out2 = c()
for(m in 1:length(annot_names)){
  out2 = rbind(out2, run_enrichment_analysis(annot_cell, results_cell, annotation = annot_names[m],
                                             traits = all_traits, index_in_results = 1))
}

rownames(out2) = annot_names
save(out2, file = paste0("/n/groups/price/kushal/LDSC-Average/output/", cell, "_enrich.rda"))
